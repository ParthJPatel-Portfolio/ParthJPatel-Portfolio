# 🔐 Cybersecurity Portfolio

<div align="center">

**Cybersecurity Graduate | Defensive Security | Incident Response | Threat Analysis**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/parthjigneshpatel)
[![GitHub](https://img.shields.io/badge/GitHub-Portfolio-black?style=flat&logo=github)](https://github.com/ParthJPatel-Portfolio)
[![ISC2 Certified](https://img.shields.io/badge/ISC²-CC%20Certified-red?style=flat)](https://www.credly.com/badges/d5829c5f-2c84-4810-85d1-ec5d2be11be1/public_url)
[![Security+](https://img.shields.io/badge/CompTIA-Security%2B%20(In%20Progress)-orange?style=flat)](https://www.comptia.org/)

</div>

---

## 🎯 About Me

Cybersecurity graduate with a background in Computer Engineering and a strong passion for **defensive security, SOC operations, threat detection, vulnerability assessment, and incident response**. 

I have developed hands-on experience through cybersecurity projects, cyber range simulations, vulnerability assessments, and threat investigations using industry-standard tools like **Splunk, Wireshark, Nessus, Nmap, and Kali Linux**. I enjoy learning by building and investigating, whether it's analyzing security risks, mapping threats to **MITRE ATT&CK**, conducting comprehensive security assessments, or working through hands-on labs and CTF challenges.

---

## 🚀 Featured Projects

### 1. **Cloud ERP Security Architecture & Risk Assessment**
   - **Tools:** Security Controls: AWS, IAM, Zero Trust, DevSecOps, NIST AI Risk Management Framework
   - **Focus:** Security architecture design for cloud-native environments, defense-in-depth design, AI security governance
   - 📂 [View Project](https://github.com/ParthJPatel-Portfolio/ERP-Security-Architecture-Case-Study)
   - **Key Deliverables:** 10+ security controls, 3-phase security roadmap, security architecture design

### 2. **Windows Server Vulnerability Assessment & Remediation**
   - **Tools:** Nmap, Tenable Nessus, Kali Linux, VMware
   - **Focus:** Vulnerability identification, risk prioritization, remediation strategies
   - 📂 [View Full Report](./Vulnerability%20Management/Vulnerability%20Assessment%20%26%20Security%20Analysis%20of%20Windows%20Server%202012%20and%202016%20Virtual%20Machines.pdf)
   - **Key Results:** Identified 15+ open services, 8 vulnerabilities

### 3. **ML-Based Intrusion Detection System**
   - **Tools:** Weka, Machine Learning, Network Analysis
   - **Focus:** Anomaly detection, threat classification, network traffic analysis
   - 📂 [View Repository](https://github.com/ParthJPatel-Portfolio/Machine-Learning-Based-Intrusion-Detection-System)
   - **Key Capability:** Automated detection of suspicious network patterns using supervised learning

### 4. **Network Forensics & Security Incident Investigation**
   - **Tools:** Wireshark, Kali Linux, MITRE ATT&CK, PCAP Analysis
   - **Focus:** Network traffic analysis, incident timeline reconstruction, attacker behavior mapping
   - 📂 [View Repository](https://github.com/ParthJPatel-Portfolio/Network-Forensics-Incident-Investigation)
   - **Key Deliverables:** Forensic evidence extraction, attack timeline reconstruction, MITRE mapping

### 5. **Secure Messaging Simulation**
   - **Tools:** Cryptography, Python, Protocol Design
   - **Focus:** Cryptographic protocol implementation, secure communication design
   - 📂 [View Repository](https://github.com/ParthJPatel-Portfolio/secure-messaging-simulation)

### 6. **Healthcare Risk Assessment Report**
   - **Tools:** NIST RMF, CIS Controls v8.1, Risk Analysis
   - **Focus:** Comprehensive risk assessment for healthcare environments
   - 📂 [View Full Report](./Risk%20Management%20Projects/Hypothetical%20Walk-in%20Clinic%20Risk%20Assessment%20Report.pdf)
   - **Key Deliverables:** 70+ page assessment, 15+ asset analysis, control recommendations

---

## 💻 Technical Skills

### **Security Operations & Incident Response**
![Splunk](https://img.shields.io/badge/Splunk-SIEM-blue?style=flat) ![Wazuh](https://img.shields.io/badge/Wazuh-SIEM-green?style=flat) ![Incident Response](https://img.shields.io/badge/Incident%20Response-SOC-red?style=flat) ![Threat Analysis](https://img.shields.io/badge/Threat%20Analysis-MITRE%20ATT%26CK-orange?style=flat)

### **Vulnerability & Risk Management**
![Nessus](https://img.shields.io/badge/Tenable%20Nessus-Scanning-blue?style=flat) ![Nmap](https://img.shields.io/badge/Nmap-Reconnaissance-green?style=flat) ![Risk Assessment](https://img.shields.io/badge/Risk%20Assessment-NIST%20RMF-red?style=flat) 

### **Network & Web Security**
![Wireshark](https://img.shields.io/badge/Wireshark-Network%20Analysis-blue?style=flat) ![Burp Suite](https://img.shields.io/badge/Burp%20Suite-Web%20App%20Security-green?style=flat) ![OWASP Top 10](https://img.shields.io/badge/OWASP-Top%2010-red?style=flat) ![TCP/IP](https://img.shields.io/badge/TCP%2FIP-Networking-orange?style=flat)

### **Offensive Security & Labs**
![Kali Linux](https://img.shields.io/badge/Kali%20Linux-Penetration%20Testing-blue?style=flat) ![Metasploit](https://img.shields.io/badge/Metasploit-Exploitation-green?style=flat) ![Caldera](https://img.shields.io/badge/Caldera-Adversary%20Simulation-red?style=flat) ![TryHackMe](https://img.shields.io/badge/TryHackMe-Cybersecurity%20Labs-orange?style=flat)

### **Cloud & Infrastructure**
![AWS](https://img.shields.io/badge/AWS-Cloud%20Security-blue?style=flat) ![GCP](https://img.shields.io/badge/GCP-Cloud%20Platform-green?style=flat) ![IAM](https://img.shields.io/badge/IAM-Identity%20%26%20Access-red?style=flat) ![Zero Trust](https://img.shields.io/badge/Zero%20Trust-Architecture-orange?style=flat)

### **Security Frameworks & Standards**
![NIST](https://img.shields.io/badge/NIST-Cybersecurity%20Framework-blue?style=flat) ![CIS Controls](https://img.shields.io/badge/CIS%20Controls-v8.1-green?style=flat) ![MITRE ATT&CK](https://img.shields.io/badge/MITRE-ATT%26CK-red?style=flat) ![NIST RMF](https://img.shields.io/badge/NIST%20RMF-Risk%20Management-orange?style=flat)

### **Operating Systems & Languages**
![Linux](https://img.shields.io/badge/Linux-blue?style=flat) ![Windows](https://img.shields.io/badge/Windows-green?style=flat) ![Python](https://img.shields.io/badge/Python-Scripting-red?style=flat) 

---

## 🎓 Education

**Master of Cybersecurity**  
Toronto Metropolitan University | *September 2025 - August 2026*

**Bachelor of Engineering, Computer Engineering**  
York University | *September 2021 - April 2025*

---

## 🏆 Certifications & Training

- ✅ **ISC² Certified in Cybersecurity (CC)** — September 2025
- 🔄 **CompTIA Security+** — In Progress (Expected September 2026)
- ✅ **CSPT 1000** – Fundamentals of Penetration Testing
- ✅ **CSPT 1020** – Web Application Penetration Testing
- ✅ **TryHackMe 2026: An AI Odyssey CTF** — Certificate of Participation

---

## 📚 TryHackMe Progress

**Completed Rooms:**
- ✅ HTTP in Detail — [Writeup](./web-application-penetration-testing-notes/HTTP%20in%20Detail.md)
- ✅ Web Application Basics — [Writeup](./TryHackMe%20Writeups/Web%20Application%20Basics.md)
- ✅ Putting it All Together — [Writeup](./TryHackMe%20Writeups/Putting%20it%20all%20together.md)
- ✅ Burp Suite Basics
- ✅ Hacker Holidays CTF Warmup: The Brochure

**CTF Writeups:**
- 🏁 [TryHackMe CTF Writeups & Solutions](https://github.com/ParthJPatel-Portfolio/TryHackMe-CTF-Writeups)

---

## 🎯 Career Goals

- **Primary Focus:** SOC Analyst / Security Analyst role in a dynamic security operations environment
- **Skill Development:** Strengthen log analysis, threat detection, and incident response capabilities
- **Portfolio Building:** Continue developing hands-on experience through real-world security projects and CTF challenges
- **Industry Impact:** Contribute to organizations' security posture through proactive threat detection and vulnerability management

---

## 📞 Connect With Me

- **LinkedIn:** [linkedin.com/in/parthjigneshpatel](https://www.linkedin.com/in/parthjigneshpatel)
- **GitHub:** [github.com/ParthJPatel-Portfolio](https://github.com/ParthJPatel-Portfolio)

---

<div align="center">

**Always learning. Always improving. Always securing.** 🔐

</div>
